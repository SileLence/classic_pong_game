...
public class ObjectRenderer {
    ...
    private void drawPlatforms(Platform[] platforms, SpriteBatch batch) {
        for (Platform platform : platforms) {
            batch.draw(platform.getTexture(), platform.getX(), platform.getY());
        }
    }
    ...
    private void drawBallTail(Ball ball, SpriteBatch batch) {
        ...
        for (int index = 0; index < trailsNumber; index++) {
            ...
            float textureX = ball.getTexture().getWidth();
            float textureY = ball.getTexture().getHeight();

            float lastIndex = (float) (index + 1) / trailsNumber;
            float scale = 0.4f + 0.6f * lastIndex;
            float alpha = 0.2f * lastIndex;
            float renderW = textureX * scale;
            float renderH = textureY * scale;

            batch.setColor(1f, 1f, 1f, alpha);
            batch.draw(ball.getTexture(), trail.x - renderW / 2f, trail.y - renderH / 2f, renderW, renderH);
        }
        // restore alpha
        batch.setColor(1f, 1f, 1f, 1f);
    }
    ...
    private void drawBall(Ball ball, SpriteBatch batch) {
        batch.draw(ball.getTexture(), ball.getX() - ball.getRadius(), ball.getY() - ball.getRadius());
    }
}
